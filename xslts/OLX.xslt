  ...
  <xsl:template name="Listing" match="AD">
    <Listing xmlns="http://www.vivareal.com/schemas/1.0/VRSync">
      ...
      <Details>
        ...
        <PropertyType>
          <xsl:choose>
            <xsl:when test="contains(CATEGORY, '367')">Apartamento - Casa à venda</xsl:when>
            <xsl:when test="contains(CATEGORY, '410')">Terrenos</xsl:when>
            <xsl:when test="contains(CATEGORY, '363')">Casa - Apartamento para aluguel</xsl:when>
            <xsl:when test="contains(CATEGORY, '301')">Quartos em aluguel - Companheiros de quarto</xsl:when>
            <xsl:when test="contains(CATEGORY, '388')">Aluguel por temporada</xsl:when>
            <xsl:when test="contains(CATEGORY, '415')">Pontos Comerciais para Alugar - Vender</xsl:when>
            <xsl:when test="contains(CATEGORY, '302')">Vagas de estacionamento</xsl:when>
            <xsl:when test="contains(CATEGORY, '368')">Escritórios - Locais de comercio</xsl:when>
