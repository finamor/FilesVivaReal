...
<xsl:variable name="descriptionVez" select="'0'" />
...
<xsl:template name="imoveis" match="Properties">
	<ListingDataFeed xmlns="http://www.vivareal.com/schemas/1.0/VRSync"
		...
		<Header>
			<Provider>Remax</Provider>
			<Email>remax@remax.com.br</Email>
		</Header>
		<Listings>
...
<Listing xmlns="http://www.vivareal.com/schemas/1.0/VRSync">
	<ListingID>
       <xsl:value-of select="IntegratorPropertyID" />
   	</ListingID>
	<TransactionType>
		<xsl:choose>
			<xsl:when test="TransactionType [@TransactionType = '2']">
				<xsl:text>For Sale</xsl:text>
			</xsl:when>
		</xsl:choose>
	</TransactionType>
	<Title>
	    <xsl:for-each select="PropertyDescriptions/PropertyDescription">
		...
		<Details>
			<PropertyType>
		<xsl:choose>
			<xsl:when test="CommercialResidential = '1'"> <!-- Define Residencial -->
				<xsl:choose>
					<xsl:when test="ApartmentNumber != '0'">
						<xsl:text>Residential / Apartment</xsl:text>
					</xsl:when>
					<xsl:when test="NumberOfBedrooms != '0'">
						<xsl:text>Residential / Home</xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:text>Residential / Land</xsl:text>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
				<xsl:choose>
					...
					<xsl:when test="NumberOfBathrooms != '0'">
	...
	<Description>
         <xsl:for-each select="PropertyDescriptions/PropertyDescription">
         	<xsl:if test="$descriptionVez = '0'">
           	<xsl:call-template name="Descricao" />
           	<xsl:variable name="$descriptionVez" select="'1'" />
           </xsl:if>
         </xsl:for-each>
       </Description>
			<xsl:if
				test="PrecoVenda != '0,00' and PrecoVenda != '0.00' and PrecoVenda != '0' and PrecoVenda != ''">
				<ListPrice currency="BRL">
					<xsl:choose>
						<xsl:when test="contains(PrecoVenda, ',')">
							<xsl:call-template name="round">
								<xsl:with-param name="value"
									select="translate(PrecoVenda, '.', '')" />
							</xsl:call-template>
						</xsl:when>
						<xsl:when test="contains(PrecoVenda, '.')">
				...
				<RentalPrice currency="BRL" period="Daily">
					<xsl:choose>
						<xsl:when test="contains(PrecoLocacaoTemporada, ',')">
							<xsl:call-template name="round">
								<xsl:with-param name="value"
									select="translate(PrecoLocacaoTemporada, '.', '')" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
				...
				<RentalPrice currency="BRL" period="Daily">
					<xsl:choose>
						<xsl:when test="contains(PrecoTemporada, ',')">
							<xsl:call-template name="round">
								<xsl:with-param name="value"
									select="translate(PrecoTemporada, '.', '')" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
			...
			<xsl:if
				test="PrecoCondominio != '0,00' and PrecoCondominio != '0.00' and PrecoCondominio != '0' and PrecoCondominio != ''">
				<PropertyAdministrationFee currency="BRL">
					<xsl:choose>
						<xsl:when test="contains(PrecoCondominio, ',')">
							<xsl:call-template name="round">
								<xsl:with-param name="value"
									select="translate(PrecoCondominio, '.', '')" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
...
<xsl:template name="Descricao"
	match="PropertyDescriptions/PropertyDescription/DescriptionType [@DescriptionType = '4']">
	<xsl:if test="DescriptionType [@DescriptionType = '4']">
		<xsl:value-of select="DescriptionText" />
	</xsl:if>
</xsl:template>
<xsl:template name="Title"
	match="PropertyDescriptions/PropertyDescription/DescriptionType [@DescriptionType = '6']">
	<xsl:if test="DescriptionType [@DescriptionType = '6']">
		<xsl:value-of select="DescriptionText" />
	</xsl:if>
</xsl:template>
</xsl:stylesheet>